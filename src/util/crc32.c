...
#if 0 // Table above was generated by this
static uint32_t
...
cmd_crcbytes(cli_t *cli, int argc, char **argv)
{
  ...
  printf("Computing CRC over: ");
  for(int i = 0; i < argc; i++) {
    buf[i] = atoix(argv[i]);
    printf("%02x ", buf[i]);
  }

  uint32_t crc = crc32(0, buf, argc);
  printf("= 0x%x\n", crc);
  return 0;
}


CLI_CMD_DEF("crcbytes", cmd_crcbytes);
...
cmd_crc(cli_t *cli, int argc, char **argv)
{
  ...
  cli_printf(cli, "0x%08x\n", crc);
  return 0;
}


CLI_CMD_DEF("crc", cmd_crc);
...
cmd_crc_test(cli_t *cli, int argc, char **argv)
{
  ...
  const uint32_t expected = 0xaeef2a50;

  cli_printf(cli, "crc(\"%s\"): 0x%08x == 0x%08x%s\n", s,
             crc, expected, crc == expected ? " OK" : " FAIL");
  return 0;
}


CLI_CMD_DEF("crc-test", cmd_crc_test);
#endif
