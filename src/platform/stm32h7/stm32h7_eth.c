...
{
  return mii_read((stm32h7_eth_t *)eni, reg);
}

static error_t
edc_mii_write(ether_netif_t *eni, uint16_t reg, uint16_t value)
{
  mii_write((stm32h7_eth_t *)eni, reg, value);
  return 0;
}
...
static void
stm32h7_thread(stm32h7_eth_t *se, gpio_t phyrst,
                 ...
                 int phy_addr, ethphy_mode_t mode, int flags)
{
  ...
  thread_createv(stm32h7_thread, 512, "eth", 0, 4, se, phyrst, mode, flags);
}
