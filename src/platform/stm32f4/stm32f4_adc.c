...
#define MOTOR_TEMP       ADC12_IN9     // GPIO_PB(1)

#define INVERTER_TEMP_A  ADC12_IN14    // GPIO_PC(4)
#define INVERTER_TEMP_B  ADC12_IN7     // GPIO_PA(7)
#define INVERTER_TEMP_C  ADC12_IN6     // GPIO_PA(6)


static uint16_t data[5];

static int
cmd_adcdma(cli_t *cli, int argc, char **argv)
{
  gpio_conf_analog(MOTOR_TEMP);
  gpio_conf_analog(INVERTER_TEMP_A);
  gpio_conf_analog(INVERTER_TEMP_B);
  gpio_conf_analog(INVERTER_TEMP_C);

  uint32_t dmainst = stm32f4_adc_multi(1,
                                       (1 << 6) |
                                       (1 << 7) |
                                       (1 << 9) |
                                       (1 << 14) |
  ...
  stm32f4_adc_transfer(1, dmainst, &data[0]);
