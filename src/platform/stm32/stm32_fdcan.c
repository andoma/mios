...
void
stm32_fdcan_init(uint32_t reg_base, uint32_t ram_base,
                 int irq, const struct dsig_filter *output_filter)
{
  fdcan_t *fc = calloc(1, sizeof(fdcan_t));
  fc->reg_base = reg_base;
  fc->ram_base = ram_base;
  ...
  irq_enable_fn_arg(irq, IRQ_LEVEL_NET, stm32_fdcan_irq, fc);
}
