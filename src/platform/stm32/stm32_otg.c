...
#define OTG_GOTGCTL  (OTG_BASE + 0x000)
...
#define OTG_GRXFSIZ  (OTG_BASE + 0x024)
#define OTG_NPTXFSIZ (OTG_BASE + 0x028)
#define OTG_GCCFG    (OTG_BASE + 0x038)

#define OTG_DIEPTXF(x) (OTG_BASE + 0x100 + 0x4 * (x))

#define OTG_DCFG     (OTG_BASE + 0x800)
#define OTG_DCTL     (OTG_BASE + 0x804)
#define OTG_DSTS     (OTG_BASE + 0x808)
#define OTG_DIEPMSK  (OTG_BASE + 0x810)
#define OTG_DOEPMSK  (OTG_BASE + 0x814)
#define OTG_DAINT    (OTG_BASE + 0x818)
#define OTG_DAINTMSK (OTG_BASE + 0x81c)

#define OTG_PCGCCTL  (OTG_BASE + 0xe00)

#define OTG_DIEPCTL(x)  (OTG_BASE + 0x900 + 0x20 * (x))
#define OTG_DIEPINT(x)  (OTG_BASE + 0x908 + 0x20 * (x))
#define OTG_DIEPTSIZ(x) (OTG_BASE + 0x910 + 0x20 * (x))
#define OTG_DTXFSTS(x)  (OTG_BASE + 0x918 + 0x20 * (x))

#define OTG_DOEPCTL(x)  (OTG_BASE + 0xb00 + 0x20 * (x))
#define OTG_DOEPINT(x)  (OTG_BASE + 0xb08 + 0x20 * (x))
#define OTG_DOEPTSIZ(x) (OTG_BASE + 0xb10 + 0x20 * (x))

#define OTG_FIFO(x)  (OTG_BASE + 0x1000 + (x) * 0x1000)

#define OTG_GINT_OEPINT   (1 << 19)
#define OTG_GINT_IEPINT   (1 << 18)
#define OTG_GINT_USBRST   (1 << 12)
#define OTG_GINT_ENUMDNE  (1 << 13)
#define OTG_GINT_USBSUSP  (1 << 11)
#define OTG_GINT_ESUSP    (1 << 10)
#define OTG_GINT_NPTXFE   (1 << 5)
#define OTG_GINT_RXFLVL   (1 << 4)
#define OTG_GINT_SOF      (1 << 3)

#define OTG_DOEP_STUP     (1 << 3)
#define OTG_DOEP_XFRC     (1 << 0)

#define OTG_DIEP_TOC      (1 << 3)
#define OTG_DIEP_XFRC     (1 << 0)
...
{
  reg_wr(OTG_DIEPTSIZ(ep),
         (1 << 19) |
         len);
  reg_or(OTG_DIEPCTL(ep),
         (1 << 31) |
         (1 << 26));
}
...
{
  if(ep != 0)
    while(reg_rd(OTG_DIEPCTL(ep)) & (1 << 31)) {}

  const size_t avail_words = reg_rd(OTG_DTXFSTS(ep));
  if(avail_words == 0) {
...
uc_ep_write(device_t *dev, usb_ep_t *ue, const uint8_t *buf, size_t len)
{
  ...
  const int avail_words = reg_rd(OTG_DTXFSTS(ep));
  ...
  if(((intptr_t)buf & 0x3) == 0) {
    for(; i + 3 < len; i += 4) {
      reg_wr(OTG_FIFO(ep), *(uint32_t *)&buf[i]);
    }
  }
...
ep_cnak(int ep)
{
  reg_wr(OTG_DOEPCTL(ep),
         reg_rd(OTG_DOEPCTL(ep)) |
         (1 << 26));
}
...
{
  reg_set_bits(OTG_DCFG, 4, 7, addr);
}
...
ep0_rx(device_t *d, usb_ep_t *ue, uint32_t bytes, uint32_t flags)
{
  ...
  if(flags & USB_COMPLETED_IS_SETUP) {
    uc->uc_ep0_out.setup_words[0] = reg_rd(OTG_FIFO(0));
    uc->uc_ep0_out.setup_words[1] = reg_rd(OTG_FIFO(0));
    ep0_handle_setup(uc);
  } else {
...
{
  uint32_t daint = reg_rd(OTG_DAINT);

  for(int ep = 0; ep < uc->uc_num_endpoints; ep++) {
    if(!((1 << ep) & daint)) // replace with ffs()
      continue;

    uint32_t diepint = reg_rd(OTG_DIEPINT(ep));
    reg_wr(OTG_DIEPINT(ep), diepint);
...
handle_oepint(usb_ctrl_t *uc)
{
  ...
  for(int i = 0; i < uc->uc_num_endpoints; i++) {
    ...
    uint32_t doepint = reg_rd(OTG_DOEPINT(i));
    reg_wr(OTG_DOEPINT(i), doepint);
...
handle_reset(usb_ctrl_t *uc)
{
  ...
  for(int ep = 0; ep < uc->uc_num_endpoints; ep++) {
    ...
    reg_clr_bit(OTG_DOEPCTL(ep), 15);

    // Clear all pending IRQs
    reg_wr(OTG_DIEPINT(ep), 0xffffffff);
    reg_wr(OTG_DOEPINT(ep), 0xffffffff);

    // Set SNAK bit
    reg_wr(OTG_DOEPCTL(ep), (1 << 27));
  ...
  reg_wr(OTG_DAINTMSK, 0x00010001);

  // Setup EP0 for 8 byte transfers
  reg_wr(OTG_DOEPTSIZ(0),
         (1 << 29) |
         (1 << 19) |
         8);

  reg_wr(OTG_DOEPCTL(0),
         (1 << 26) |
         (1 << 31) | 3);
}
...
static void
stm32_otg_irq(void *arg)
{
  usb_ctrl_t *uc = arg;

  while(1) {

    uint32_t gint = reg_rd(OTG_GINTSTS) & reg_rd(OTG_GINTMSK);
    if(gint == 0)
      break;

    if(gint & OTG_GINT_USBRST) {
      handle_reset(uc);
      reg_wr(OTG_GINTSTS, OTG_GINT_USBRST);
      uc->uc_resets++;
    }

    if(gint & OTG_GINT_ENUMDNE) {
      handle_enum_done(uc);
      reg_wr(OTG_GINTSTS, OTG_GINT_ENUMDNE);
      uc->uc_enumerations++;
    }

    // Read from FIFO
    if(gint & OTG_GINT_RXFLVL) {
      const uint32_t rspr = reg_rd(OTG_GRXSTSP);
    ...
    if(gint & OTG_GINT_OEPINT) {
      handle_oepint(uc);
    }

    if(gint & OTG_GINT_IEPINT) {
      handle_iepint(uc);
    }


    if(gint & OTG_GINT_ESUSP) {
      // Early suspend
      reg_wr(OTG_GINTSTS, OTG_GINT_ESUSP);

      if(reg_rd(OTG_DSTS) & 8) {
...
usb_fini(void)
{
  reg_set_bits(OTG_DCTL, 1, 1, 1); // Soft disconnect
}

static void
otg_init_regs(usb_ctrl_t *uc)
{
  reg_wr(OTG_GINTMSK, 0);          // Disable all interrupts
  reg_wr(OTG_DIEPMSK, 0);
  reg_wr(OTG_DOEPMSK, 0);
...
static void
uc_ep_write1(device_t *dev, usb_ep_t *ue,
...
static void
uc_ep_cnak(device_t *dev, usb_ep_t *ue)
{
  const uint32_t ep = ue->ue_address & 0x7f;
  ep_cnak(ep);
}
...
static int
uc_ep_avail_bytes(device_t *dev, usb_ep_t *ue)
{
  const uint32_t ep = ue->ue_address & 0x7f;
  if(reg_rd(OTG_DIEPCTL(ep)) & (1 << 31))
    return 0;
...
static const usb_ctrl_vtable_t otg_vtable =
{
  .read = uc_ep_read,
  .write = uc_ep_write,
  .write1 = uc_ep_write1,
  .cnak = uc_ep_cnak,
  .avail_bytes = uc_ep_avail_bytes,
};
    ...
    for(size_t j = 0; j < ui->ui_num_endpoints; j++)  {
      ...
      if(ue->ue_address & 0x80) {
        ...
        if(ep_out_index == uc->uc_num_endpoints)
          panic("USB/OTG: Not enough %s endpoints", "OUT");
...
probe_endpoints(usb_ctrl_t *uc)
{
  ...
  for(int i = 1; i < MAX_NUM_ENDPOINTS; i++) {
    if(!(reg_rd(OTG_DIEPTXF(i)) & 0xffff))
      break;
    uc->uc_num_endpoints++;
  }

  printf("USB: %d endpoints\n", uc->uc_num_endpoints);
}
...
{
  reg_set_bits(OTG_DCTL, 1, 1, 0); // No soft disconnect
}
